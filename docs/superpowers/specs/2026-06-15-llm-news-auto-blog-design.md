...
```
[스케줄러가 Claude Code를 매일 1회 깨움 — /schedule 클라우드 루틴]
   │
...
## 6. 발행 정책
- **주기**: 매일 1회 (스케줄 루틴).
- **발행량**: 1회 1~3건 (신규 가치 있는 항목만; 없으면 skip). 각 건 = ko+en 쌍.
- **중복제거**: 원문 URL + 제목 유사도로 기발행분과 비교.
- **품질 가드(AdSense)**: 원문 복붙 금지, 최소 단어수, 출처 명시, 양언어 재작성 필수.

---

## 7. 자동화 (스케줄러)
- **1순위**: `/schedule` 스킬 (클라우드 루틴, cron) — PC 비의존, 인프라 0.
- **대안**: 로컬 cron + `claude -p` (PC 상시 가동 필요).
- 실패 시: 로그 남기고 다음 주기 재시도 (silent failure 금지).
