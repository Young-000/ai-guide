...
## 🔴🔴 최우선 (2026-08-16 주간 PM 발견) — 야간 dev 사이클이 이 repo의 백로그를 소비하지 않음

> **증상**: `docs/backlog.md` 미완 30여 개, 그중 트래픽 카운터는 **5주 연속 이월**(page_views 여전히 0 rows). 그런데 최근 7일 커밋이 **전부 `content(news)` 발행뿐 — feature/dev 커밋 0.** 즉 리필은 매주 되는데 구현이 한 번도 안 붙는다.
>
> **함의**: 발행 크론(publish-local)만 이 repo를 돌고, `venture-cycle` dev 단계는 ai-guide를 실제로 집어 구현·머지한 흔적이 없다. 태스크를 더 잘게 쪼개도(이번 주 함수 단위까지 축소) 소비 파이프라인이 안 돌면 6주째 이월된다.
>
> **오너 확인 요청**: `VENTURE_DOMAINS=teamY` 야간 사이클이 ai-guide를 대상에 포함하는지 / backlog.md(소문자)를 백로그 소스로 인식하는지 / verify.sh 존재 여부. 소비가 재개되면 이번 주 리필의 카운터 2태스크가 측정 블로커를 즉시 해소한다.
